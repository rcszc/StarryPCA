...
#include <io.h>
#include <fcntl.h>
	std::atomic<bool> LOG_PRINT_SWITCH = true;
	...
	void PushLogProcess(const LOGLABEL& label, const std::string& module_name, const std::string& logstr_text) {
		...
		// => read logger chache.
		GLOBAL_LOG_CACHE.push_back(LOGCONS::ReadLogCache::LogCache(FmtLog, module_name, label));
...
namespace LOGGER_FILESYS {
	...
	void logprocess_write_file_eventloop(const char* folder) {
		...
		LOGCONS::PushLogger(LogInfo, MODULE_LABEL_LOGGER, "create log_file: %s", LogFileName.c_str());
	...
	bool StartLogFileProcess(const char* folder) {
		if (!filesystem::exists(folder) || !filesystem::is_directory(folder)) {
			LOGCONS::PushLogger(LogError, MODULE_LABEL_LOGGER, "invalid folder: %s", folder);
			return false;
		}
		// init logger_thread.
		try {
			LogProcessThread = new thread(logprocess_write_file_eventloop, folder);
			LOGCONS::PushLogger(LogInfo, MODULE_LABEL_LOGGER, "start thread success.");
		}
		catch (const exception& err) {
			LOGCONS::PushLogger(LogError, MODULE_LABEL_LOGGER, "start thread error: %s", err.what());
			return false;
		}
		return true;
	}
	...
	bool FreeLogFileProcess() {
		...
		try {
			...
			LOGCONS::PushLogger(LogInfo, MODULE_LABEL_LOGGER, "free thread success.");
		}
		catch (const exception& err) {
			LOGCONS::PushLogger(LogError, MODULE_LABEL_LOGGER, "free thread error: %s", err.what());
			return false;
		}
		return true;
	}
